...
class DSQLInsert : DSQLStatement {
	...
	this(string table, string[string] values = null) {
		this();
		from(table).values(values);
	}

	this(string table = "", string names = "", string values = "") {
		this();
		from(table).values(names, values);
	}
...
auto SQLInsert() {
	return new DSQLInsert;
}

auto SQLInsert(string table, string[string] values) {
	return new DSQLInsert(table, values);
}

auto SQLInsert(string table = "", string names = "", string values = "") {
	return new DSQLInsert(table, names, values);
}

unittest {
	writeln("Testing ", __MODULE__);

	assert(SQLInsert.table("tab").columns("id, name")
			.values("1, " test1"") == "INSERT INTO tab (id, name) VALUES(1, " test1")");
}
