...
class SQLChunkProcessor : Processor {
  ...
  auto process(sqlOut) {
    ...
    if (!sqlOut["HAVING"].isEmpty) {
      ...
      sqlOut["HAVING"] = myProcessor.process(sqlOut["HAVING"], sqlOut.isSet("SELECT") ? sqlOut["SELECT"] : [
        ]);
    }
    if (!sqlOut["SET"].isEmpty) {
      auto myProcessor = new SetProcessor(this.options);
      sqlOut["SET"] = myProcessor.process(sqlOut["SET"], sqlOut.isSet(
          "UPDATE"));
    }
    if (
      !sqlOut["DUPLICATE"]
